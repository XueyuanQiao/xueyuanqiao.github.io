...
页面还包含一个不可见且无声的 `[data-music-warmup]` 原生音频节点。它只负责在高清星图请求结束后预热博客播放器的第一首曲目，不属于视觉分层，也不能在宇宙页直接出声；用户提前点击博客入口时仍会立即启动预热。
...
1. 浏览器解析 CSS 时立即显示 Preview 背景，并通过模糊遮罩、扫描光和“正在接收三体信号”场景明确表达正在加载。副标题以“来自 4.37 光年外 · 信号译码中，请不要回答”延续三体语境，但不替代连接、进度和解析状态；主标题、副标题、进度区和轨道按清晰可读的层级放大，可见状态统一使用较短的“高清星图”表述。
2. `<head>` 只高优先级预载 5.6 KB Preview。JavaScript 先读取 `MAX_TEXTURE_SIZE`，再选择 6000 × 3000 原图或 4096 × 2048 兼容图，避免低纹理设备先下载无用的 6.8 MB 桌面原图。
3. 最终资源使用同源 `XMLHttpRequest` 接收 Blob；响应提供 `Content-Length` 时按实际字节更新进度，缺少长度时只显示已接收体积和不定进度，不伪造百分比。
4. Preview 完成解码并上传为 WebGL 纹理后添加 `.panorama-preview-ready` 和 `.particle-ready`，立刻开放拖曳、惯性、缩放、方向键和粒子渲染，同时把状态更新为“预览已可拖曳”。
5. 原始图接收完成后依次显示“解析星域”“星图校准”和“高清星海已就绪”；上传后添加 `.panorama-high-ready`，短暂停留完成反馈，再添加 `.panorama-loading-dismissed` 让加载场景淡出。
6. 高清请求超过约 6.5 秒时明确提示信号仍在接收；升级失败时短暂说明当前保留可交互 Preview，然后结束加载场景。
...
| `.panorama-loading-dismissed` | 完成或失败反馈已停留足够时间，加载场景开始淡出 |
| `.particle-ready` | 渲染循环和交互已经启动 |
| `.panorama-upgrade-failed` | 原图升级失败，保留可交互 Preview，并显示一次明确的失败反馈 |
...
- 高清星图请求成功或失败后，JavaScript 才在浏览器空闲阶段为第一首站内音乐设置 `preload="auto"`，避免 4 MB 级音频在弱网下和星图争抢带宽；用户提前点击入口时会立即启动预热并记录播放意图。整个跃迁过程保持无声，博客页仍以自身实际缓冲水位决定何时出声。
...
- 高清加载轨道使用 `role="progressbar"`；读屏只在连接、预览可拖曳、慢网提示、25% 分段、解析、完成或失败时收到状态更新，避免逐数据块播报。
...
- `/` 首屏出现带模糊和扫描光的 Preview 过渡背景，以及“正在接收三体信号”场景；下方必须直接写明正在加载高清宇宙星象图，并显示目标分辨率、体积和可感知的进度轨道。
- 冷缓存慢网下，百分比和“已接收 / 总体积”应随真实字节持续增长；同一最终星图只能出现一次网络请求，响应无 `Content-Length` 时不得显示伪造百分比。
- Preview 上传后出现 `.panorama-preview-ready` 和 `.particle-ready`；状态变为“预览已可拖曳”，原图仍在下载时鼠标和触摸拖曳已经能移动模糊全景。
- 最终依次出现解析、校准和完成反馈，再出现 `.panorama-high-ready` 与 `.panorama-loading-dismissed`；高清画面在当前视角完成升级且没有中清停留状态。
- 阻断最终星图时出现一次“高清星图接收中断”反馈，随后保留可交互 Preview，不得无限显示接收中。
...
- 宇宙页和跃迁过程全程无声；最终星图请求结束前 `[data-music-warmup]` 不应抢占网络，结束后才出现 `currentSrc` 和递增的 `buffered`。进入 `/blog/` 后音频在 `document.readyState === 'complete'` 前保持暂停，完整加载后再接续安全缓冲与播放。
...
- Preview 使用高优先级图片预载；最终资源由 JavaScript 在确认 GPU 纹理上限后发起同源 XHR，不得再增加一条会造成重复下载的图片预载。
- 音乐预热只在最终星图请求结束后通过 `requestIdleCallback`（带超时降级）启动，不得和高清星图竞争首屏带宽，也不得在宇宙页播放声音。
...
- 放大深空接收场景的主副标题、进度状态、规格信息和进度轨道，并加宽信息区；可见状态文案缩短为“高清星图”，提升桌面与窄屏可读性。
- 加载场景副标题调整为“来自 4.37 光年外 · 信号译码中，请不要回答”，以低层级文案加入三体梗，同时保留明确的译码状态。
- 保留“正在接收三体信号”主文案，新增高清宇宙星象图的真实字节进度、分辨率与体积、慢网提示、解析校准和完成反馈。
- 最终星图改为先按 GPU 能力选择资源再请求，移除无条件桌面原图预载；音乐预热延后到星图请求结束，避免弱网竞争。
