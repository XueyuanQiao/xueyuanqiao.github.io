...
- JavaScript 在浏览器空闲阶段为第一首站内音乐设置 `preload="auto"`，让同源 HTTP 媒体缓存提前积累；点击入口时会确保预热已经启动。博客页仍会根据自身实际缓冲水位决定何时出声，不把预热记录当作安全播放保证。
...
- 宇宙页全程无声；打开一段时间后检查 `[data-music-warmup]` 应已有 `currentSrc` 和递增的 `buffered`，进入 `/blog/` 后播放器应接续加载并等待安全缓冲水位。
