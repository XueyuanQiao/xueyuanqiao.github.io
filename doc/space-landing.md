...
| 文件 | 作用 |
| --- | --- |
| `particle-cat.html` | 页面结构、全景资源地址、博客入口、音乐预热节点、重置入口和降级内容 |
...
页面还包含一个不可见且无声的 `[data-music-warmup]` 原生音频节点。它只负责在浏览宇宙页期间预热博客播放器的第一首曲目，不属于视觉分层，也不能在宇宙页直接出声。
...
- JavaScript 在浏览器空闲阶段为第一首站内音乐设置 `preload="auto"`，让同源 HTTP 媒体缓存提前积累；点击入口时只确保预热已经启动并记录播放意图，整个跃迁过程保持无声。博客页在完整加载后，仍会根据自身实际缓冲水位决定何时出声，不把预热记录当作安全播放保证。
...
- 宇宙页和跃迁过程全程无声；打开一段时间后检查 `[data-music-warmup]` 应已有 `currentSrc` 和递增的 `buffered`，进入 `/blog/` 后音频在 `document.readyState === 'complete'` 前保持暂停，完整加载后再接续安全缓冲与播放。
...
- 音乐预热通过 `requestIdleCallback`（带超时降级）启动，不得提升到与原始高清星图相同的首屏关键优先级，也不得在宇宙页播放声音。
...
- 宇宙页入口不再在跃迁中调用音频播放，博客播放器改为整页加载完成后再启动安全缓冲和自动播放。
- 博客播放器改为默认展开，PJAX 换页保留当前展开状态。

### 2026-08-09

- 宇宙页增加第一首站内音乐的无声媒体预热；博客播放器基于实际缓冲和网络速度决定安全起播水位，避免弱网下断续播放。
