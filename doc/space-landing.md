...
- `launch-transition__canvas` 使用 Canvas 2D 实时绘制低亮度放射状星轨；引力透镜、吸积盘、火箭和文案由 CSS 动画完成，不引入额外运行时依赖。
...
- 点击博客入口后，火箭应从入口图标位置升空，出现克制的星轨加速、暗色引力透镜与“航向 Xueyuan 的技术宇宙”文案，约 1.58 秒后进入 `/blog/`；过程中不应出现刺眼白光或占满屏幕的机械圆环。
